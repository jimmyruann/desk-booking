...
describe('Map Component', () => {
  const mockLocationId = 'test';
  const mockBaseUrl = 'http://localhost/assets/floorplan';
  const mockSvgUrl = `/${mockLocationId}/map.svg`;
  const mockMapAreaJsonUrl = `/${mockLocationId}/areas.json`;
  ...
  it('should render', async () => {
    ...
    nock(mockBaseUrl).get(mockSvgUrl).reply(200, mockSvg);
    nock(mockBaseUrl).get(mockMapAreaJsonUrl).reply(200, mockMapAreaJson);

    const { container } = render(
      <Map htmlIdHook={['', mockHtmlSetState]} locationId={mockLocationId} />,
      { wrapper }
    );
